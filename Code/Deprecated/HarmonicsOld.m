function [HarmonicT, HarmonicP] = Harmonics(N)
...
for h = -(N-1)/2 : (N-1)/2
    for j = 1:3*N
        ...
        for k = -(N-1)/2 : (N-1)/2
            ...
            if k == h
                if bcheck == 1
                    ...
                    X21 = (-bc(1:2,3:end)*X22.').';
                    X11 = bc2(1:2,1:2)-bc(1:2,3:end)*X21;
                else
                    X12 = -bc(1:2,3:end)*X22;
        ...
        HarmonicT = zeros(N^3,3*N^2, 2);
        HarmonicP = zeros(N^3,3*N^2, 2);
        ...
        for jj = -(N-1)/2 : (N-1)/2
            Temp2 = D0*1i*jj*Fapprox(:, :, jj+(N+1)/2);
        ...
        HarmonicT(:,(h+(N+1)/2-1)*3*N+j,1) = (B1*M).';
        ...
        B2 = B2 + Temp4*kron(DFT,kron(DCT,chebtech2.coeffs2vals(D0*eye(N))));
        HarmonicT(:,(h+(N+1)/2-1)*3*N+j,2) = B2.';
        ...
        B1 = kron(ones(1,N),kron(CCval,ONESval)).*diag1;
        B1 = B1 + kron(ones(1,N),kron(CCval,CC2val)).*diag2;
        HarmonicP(:,(h+(N+1)/2-1)*3*N+j,1) = (B1*M).';
        ...
        Temp4 = kron(ones(1,N),kron(Aval,CC2val)).*diag6;
        B2 = B2 + Temp4*kron(DFT,kron(DCT,chebtech2.coeffs2vals(D0*eye(N))));
        HarmonicP(:,(h+(N+1)/2-1)*3*N+j,2) = B2.';
