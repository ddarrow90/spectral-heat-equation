...
K = (1:N)'./(4:2:2*N+2)';
K1 = (3:N+2)'./(4:2:2*N+2)';
R2 = spdiags([K K1], [-1 1], N, N); 
R1 = U12\(R2*U12); % Fix later.

% Construction of "r"-part of Laplacian on cylinder:
A = R2^2*D2 + R2*U12*D1;   % A[u] = r^2*u_rr + r*u_r
...
if checkparam
    ...
    Sol = 1;
    for k = -(N-1)/2 : (N-1)/2
        r2Div(:,:,k+(N+1)/2) = (R2*U02+R2^2*U12*D1)*CFSr(:,:,k+(N+1)/2)*U02.';
        r2Div(:,:,k+(N+1)/2) = r2Div(:,:,k+(N+1)/2)-k^2*R2^2*U02*CFSt(:,:,k+(N+1)/2)*U02.';
        r2Div(:,:,k+(N+1)/2) = r2Div(:,:,k+(N+1)/2)+R2^2*U02*CFSz(:,:,k+(N+1)/2)*D1.'*U12.';
...
for k = -(N-1)/2 : (N-1)/2
    ...
    AA = A - k^2*U02;
    RHSt = -(R2^2*U12*D1+R2*U02)*CFSt(:,:,k+(N+1)/2)+k*1i*R2*U02*CFSr(:,:,k+(N+1)/2);
    RHSp = -coF*CFSz(:,:,k+(N+1)/2);
    AA = AA(1:end-2,:);
    RHSt = RHSt(1:end-2,:);
    RHSp = RHSp(1:end-2,:);
    
    AAA = kron(speye(2),AA);
    RHS = [RHSt zeros(N-2,N); zeros(N-2,N) RHSp];
    %[AA, RHS] = zeroDOF(AA, RHS, bc, zeros(2,N));
    
    bAAA = bc*[1i*k*U01 R1*D1];
    bRHS = [bc*R1*U01*CFSr(:,:,k+(N+1)/2)*U01.'];
    L=[U01.';D1.'];
    bRHS = bRHS/L;
    
    bAAA1 = bc*[-R1*D1 1i*k*U01];
    bRHS1 = [bc*R1*U01*CFSt(:,:,k+(N+1)/2)*U01.'];
    L = [U01.';D1.'];
    bRHS1 = bRHS1/L;
    spy(bRHS1)
    %bRHS
    ...
    X = AAA\RHS;
    %spy(X)
    FapproxTor(:, :, k+(N+1)/2) = X(1:N,1:N);
    FapproxPol(:, :, k+(N+1)/2) = X(N+1:2*N,N+1:2*N);
end
